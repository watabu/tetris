...
public class MinoControllerScript : MonoBehaviour
{
    ...
    {
        var cells__ = new Vector3Int[cellSize, cellSize];
        for (int i = 0; i < cellSize; i++)
            for (int j = 0; j < cellSize; j++)
                cells__[i, j] = new Vector3Int(-100, -100, -100);
    ...
    Vector3Int GetInsideOffset()
    {
        for (int y = -1; y <= 1; y++)
            for (int x = -1; x <= 1; x++)
            {
                ...
                {
                    if (!IsNull(cell)&&
                        gameBoardS.IsEmpty(BoardLayer.Default, cell + new Vector3Int(x, y, 0))) count++;
                }
                if (count == 4)
